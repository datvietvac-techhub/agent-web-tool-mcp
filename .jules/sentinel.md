...
## 2024-05-18 - [Preventing Timing Attacks with secrets.compare_digest]
**Vulnerability:** The API token validation in `mcp/api.py` was using standard string comparison (`!=`) which is vulnerable to timing attacks. An attacker could theoretically guess the token by measuring the time it takes for the server to reject incorrect tokens character by character.
**Learning:** Python's standard string comparison short-circuits on the first mismatched character, meaning strings that share a common prefix take slightly longer to compare than strings that differ at the first character.
**Prevention:** Always use `secrets.compare_digest()` for comparing sensitive tokens, hashes, or passwords to ensure constant-time comparison, as specified in the Sentinel security conventions.
