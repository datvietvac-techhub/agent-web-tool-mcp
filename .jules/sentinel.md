...
## 2024-05-23 - [CRITICAL] Prevented Timing Attack in API Authentication
**Vulnerability:** The API token verification in `mcp/api.py` used a standard string inequality operator (`!=`) to compare the provided bearer token against the configured secret (`API_TOKEN`). This allowed for a potential timing attack, where an attacker could deduce the secret token character by character based on the time taken for the server to reject incorrect tokens.
**Learning:** Standard string comparisons terminate early upon encountering the first mismatching character. When used for sensitive secrets or hashes, this timing difference leaks the secret's structure.
**Prevention:** Always use `secrets.compare_digest()` for comparing sensitive tokens, hashes, or passwords to ensure constant-time comparison, neutralizing timing side channels.
